...
parser.add_argument('-a', '--all', help='Save extracted response parameters to output files too.', dest='all_params',action='store_true')
...
def initialize(request, wordlist, single_url=False):
    ...
    if not request['url']:
        ...
        request['_extracted_params'] = found if found else []
...
def main():
    ...
    try:
        ...
        for request in requests:
            ...
            if args.all_params and isinstance(these_params, list):
                extracted = request.get('_extracted_params', [])

                for p in extracted:
                    if p not in these_params:
                        these_params.append(p)
            count += 1
            mem.var['kill'] = False
